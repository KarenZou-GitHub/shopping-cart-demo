              ...
              <ul class="cart-item-list">
                <li v-for="(item,index) in carlist">
                      ...
                      <dl>
                        ...
                        <dd v-for="part in item.parts">
                          <div class="partContent">
                            <div class="left">
                                      <span  v-text="part.partsName"></span>
                            </div>
                            <div class="right">
                              <img v-bind:src="part.img" style="width:50px" v-show="part.showImg"/>
                            </div>
                          </div>
                        </dd>
                      </dl>
                  ...
                  <div class="cart-tab-2">
                    <div class="item-price">{{item.productPrice | round}}</div>
                  </div>
                    ...
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="quentity">
                          ...
                          <a href="javascript:void 0" v-on:click="changeNum(item,+1)">+</a>
                        </div>
                      </div>
                  ...
                  <div class="cart-tab-4">
                    <div class="item-price-total">{{item.productPrice * item.productQuentity | round}}</div>
                  </div>
                  <div class="cart-tab-5">
                    <div class="cart-item-opration">
                      <a href="javascript:void 0" class="item-edit-btn"
                         >
                        <svg class="icon icon-del"><use xlink:href="#icon-del"  @click="delconfirm(item,index)" ></use></svg>
                      </a>
                    </div>
                  </div>
                </li>
          ...
          <div class="cart-foot-wrap">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascript:void 0">
                  ...
                  <span @click="selectedProtectAll(true)">全选</span>
                </a>
              </div>
              <div class="item-all-del">
                <a href="javascript:void 0" class="item-del-btn" @click="selectedProtectAll(false)">
                  取消全选
                </a>
              </div>
            </div>
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: <span class="total-price">{{totalMoneySum | round}}</span>
              </div>
              <div class="next-btn-wrap">
                <a href="javascript:void(0)" class="btn btn--red">结账</a>
              </div>
            </div>
          </div>
        </div>
  ...
  methods:{
    getcarList: function () {
      this.$http.get('../static/resource/cartData.json').then(function (res) {
        this.carlist=res.body.result.list;
      })
    },
    //修改产品数量
    ...
    getTotalMount:function () {
      var _this=this;
      this.totalMoneySum=0;
      this.carlist.forEach(function (item,index) {
        if(item.isChecked){
          _this.totalMoneySum+= item.productPrice * item.productQuentity;
        }
      })
    },
    //选择产品
    selectedProtect:function (item) {
...
//            Vue.set(item,"checked",true);
    ...
    //全选 不全选
    selectedProtectAll:function (flag) {
      this.checkAll =flag;
      var _this=this;
      //遍历商品列表
      this.carlist.forEach(function(item,index){
        if(typeof item.isChecked == 'undefined'){
          _this.$set(item,"isChecked",_this.checkAll);
        }else{
    ...
    delconfirm:function(item,_index){
      if(typeof item.isChecked == 'undefined') {
        alert("您没有选择该商品！");
        return;
      }else{
...
//      $delete vue1.0已经取消这个方法
      var index =this.carlist.indexOf(this.curproduct);
