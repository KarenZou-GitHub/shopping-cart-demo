    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(address,index) in filteAddressLimit" v-bind:class="{'check':addressIndex==index}"  @click="addressIndex=index">
                <dl>
                ...
                <div class="addr-opration addr-del">
                  <a href="javascript:;" class="addr-del-btn">
                    <svg class="icon icon-del"><use xlink:href="#icon-del"  @click="delconfirm(address,index)"></use></svg>
                  </a>
                </div>
          ...
          <div class="md-modal-inner">
            ...
            <div class="md-content">
              <div class="confirm-tips">
                <p id="cusLanInfo" lan="Cart.Del.Confirm">你确认删除此配送地址信息吗?</p>
              </div>
        ...
        <div class="md-modal modal-msg md-modal-transition" id="showModalw"
             v-bind:class="{'md-show':confirmAdd}" >
          <div class="md-modal-inner">
            <div class="md-top">
              <button class="md-close" @click="confirmAdd=false">关闭</button>
            </div>
            <div class="md-content">
              <div class="confirm-tips">
              ...
              <div class="btn-wrap col-2">
                <button class="btn btn--s" id="btnModalConfirms" @click="delProduct">保存</button>
                <button class="btn btn--s btn--red" id="btnModalCancels" @click="confirmAdd=false">取消</button>
              </div>
            </div>
          </div>
        </div>
        <div class="md-overlay" id="showOverLay" v-show="confirmDelete" ></div>
        <div class="md-overlay" id="showOverLays" v-show="confirmAdd" ></div>
      </div>
    </div>
  </div>
</template>
<script>
  ...
  methods:{
    getDataList: function () {
      var _this=this;
      this.$http.get('../static/resource/address.json').then(function (res) {
